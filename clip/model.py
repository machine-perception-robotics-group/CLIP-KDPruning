...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, search: bool = False):
        super().__init__()
        #mock.pyで定義されているMultiheadAttentionを使用
    ...
    def forward(self, x: torch.Tensor):
        ...
        #↓alphaがある場合はelse以降が実行され，ニューロンにalphaをかける
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        #モデルパスのvision_layersがtupleかlistの場合は以下が実行
        if isinstance(vision_layers, (tuple, list)):
            #tupleかlistの場合はResNetベースのImageEncoderを作成
            print("ResNetベースのImageEncoderを作成")
            ...
            #tupleかlistでない場合はVisionTransformerを作成
            print("VisionTransformerを作成")
    ...
    def forward(self, image, caption, alpha, idx, KD=False, mask_update_step=False, sim_i2t_targets_log=None,sim_t2i_targets_log=None):
        with torch.no_grad():
        ...
        # normalized features
        #MSEで比較するなら正規化前の方が良いかもです↑
        image_features = image_features / image_features.norm(dim=1, keepdim=True)
        text_features = text_features / text_features.norm(dim=1, keepdim=True)
        #知識蒸留用にimage_featuresとtext_featuresを保存
        image_Fs = image_features
        text_Fs = text_features
        ...
        with torch.no_grad():
            ...
            #異なるモーダル間の知識蒸留
            """
            教師モデルの捉えたコサイン類似度の表の順位関係のみを転移させる
            """
            if sim_i2t_targets_log is not None and sim_t2i_targets_log is not None:
                if not mask_update_step:
                    sim_i2t_targets = alpha * F.softmax(sim_i2t_m, dim=1) + (1 - alpha) * sim_i2t_targets_log
                    sim_t2i_targets = alpha * F.softmax(sim_t2i_m, dim=1) + (1 - alpha) * sim_t2i_targets_log
                else:
                    sim_i2t_targets = alpha * F.softmax(sim_i2t_m, dim=1) + (1 - alpha) * sim_i2t_targets_log
                    sim_t2i_targets = alpha * F.softmax(sim_t2i_m, dim=1) + (1 - alpha) * sim_t2i_targets_log
                    """マスク更新時は知識蒸留を行わないなら以下のコードを使用"""
        ...
        #知識蒸留のためにlogits_per_imageとlogits_per_textを保存
        i2t_sim =logits_per_image
        t2i_sim =logits_per_text
        ...
        return loss_ita, image_Fs, text_Fs, i2t_sim, t2i_sim
...
def build_model(state_dict: dict, search: bool, evaluate: bool = False):
    ...
    if vit:#モデルパスにvisual.projがある場合は以下が実行
        vision_width = state_dict["visual.conv1.weight"].shape[0]
        vision_layers = len([k for k in state_dict.keys() if k.startswith("visual.") and k.endswith(".attn.in_proj_weight")])
        vision_patch_size = state_dict["visual.conv1.weight"].shape[-1]
        grid_size = round((state_dict["visual.positional_embedding"].shape[0] - 1) ** 0.5)
        image_resolution = vision_patch_size * grid_size
    else:#モデルパスにvisual.projがない場合は以下が実行（ResNetベースのImageEncoder）
