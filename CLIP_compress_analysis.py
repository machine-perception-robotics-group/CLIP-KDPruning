...
#from pruning_analysis import PruningAnalysis
...
@torch.no_grad()
def evaluation(model, data_loader, device, config,search=False, ax_epoch=0, make_sim_matrix=False,teacher_model_env=0, min_val=0, max_val=1, analyze_top_k=False):
    # test
    model.eval() 
    ...
    top_k_analysis = {}
    if analyze_top_k:
        top_k_analysis = analyze_top_k_predictions(sims_matrix.cpu().numpy(), sims_matrix.t().cpu().numpy(), 
                                                 data_loader.dataset.txt2img, data_loader.dataset.img2txt, 
                                                 texts, data_loader.dataset, k=10)
    ...
    return sims_matrix.cpu().numpy(), sims_matrix.t().cpu().numpy(), text_embeds_log, image_embeds_log, sims_matrix_log, min_val, max_val ,ax_epoch, top_k_analysis

def analyze_top_k_predictions(scores_i2t, scores_t2i, txt2img, img2txt, texts, dataset, k=10):
    """
    Top-K予測の詳細分析を行う関数
    
    Args:
        scores_i2t: 画像→テキストの類似度スコア
        scores_t2i: テキスト→画像の類似度スコア
        txt2img: テキスト→画像のマッピング
        img2txt: 画像→テキストのマッピング
        texts: テキストリスト
        dataset: データセット
        k: 分析する上位K個
    ...
    # 画像→テキストの分析
    print(f"\n=== 画像→テキスト Top-{k} 分析 ===")
    for img_idx in range(min(50, len(scores_i2t))):  # 最初の50画像を分析
        img_scores = scores_i2t[img_idx]
        top_k_indices = np.argsort(img_scores)[::-1][:k]
        correct_texts = img2txt[img_idx]
        
        # 画像IDを取得
        if hasattr(dataset, 'image_ids'):
            image_id = dataset.image_ids[img_idx]
        elif hasattr(dataset, 'image'):
            image_id = dataset.image[img_idx]
        else:
            image_id = img_idx
        
        print(f"\n画像 {img_idx} (画像ID: {image_id}):")
        print(f"正解テキスト: {[texts[i] for i in correct_texts]}")
        print(f"Top-{k} 予測:")
        
        found_correct = False
        for rank, text_idx in enumerate(top_k_indices):
            text = texts[text_idx]
            score = img_scores[text_idx]
            is_correct = text_idx in correct_texts
            
            if is_correct and not found_correct:
                print(f"  {rank+1}. ✓ {text} (スコア: {score:.4f}) - 正解!")
                found_correct = True
            else:
                print(f"  {rank+1}. {'✓' if is_correct else '✗'} {text} (スコア: {score:.4f})")
        
        if not found_correct:
            print(f"  ❌ 正解がTop-{k}に入っていません")
            #正解テキストが持つスコア
            correct_text_score = img_scores[correct_texts]
            print(f"correct_text_score {correct_text_score}")
        
        # 分析結果を保存
        analysis_results['image_to_text'][img_idx] = {
            'image_id': image_id,
            'correct_texts': [texts[i] for i in correct_texts],
            'top_k_predictions': [(texts[i], img_scores[i], i in correct_texts) for i in top_k_indices],
            'found_correct': found_correct
        }
    
    # テキスト→画像の分析
    print(f"\n=== テキスト→画像 Top-{k} 分析 ===")
    for text_idx in range(min(50, len(scores_t2i))):  # 最初の50テキストを分析
        text_scores = scores_t2i[text_idx]
        top_k_indices = np.argsort(text_scores)[::-1][:k]
        correct_img = txt2img[text_idx]
        
        # 正解画像の画像IDを取得
        if hasattr(dataset, 'image_ids'):
            correct_image_id = dataset.image_ids[correct_img]
        elif hasattr(dataset, 'image'):
            correct_image_id = dataset.image[correct_img]
        else:
            correct_image_id = correct_img
        
        print(f"\nテキスト {text_idx}: {texts[text_idx]}")
        print(f"正解画像: {correct_img} (画像ID: {correct_image_id})")
        print(f"Top-{k} 予測:")
        
        found_correct = False
        for rank, img_idx in enumerate(top_k_indices):
            score = text_scores[img_idx]
            is_correct = img_idx == correct_img
            if hasattr(dataset, 'image_ids'):
                image_id = dataset.image_ids[img_idx]
            elif hasattr(dataset, 'image'):
                image_id = dataset.image[img_idx]
            ...
            if is_correct and not found_correct:
                print(f"  {rank+1}. ✓ 画像 {img_idx} (画像ID: {image_id}) (スコア: {score:.4f}) - 正解!")
                found_correct = True
            else:
                print(f"  {rank+1}. {'✓' if is_correct else '✗'} 画像 {img_idx} (画像ID: {image_id}) (スコア: {score:.4f})")
        
        if not found_correct:
            print(f"  ❌ 正解がTop-{k}に入っていません")
            #正解画像が持つスコア
            correct_img_score = text_scores[correct_img]
            print(f"correct_img_score {correct_img_score}")
        
        # 分析結果を保存
        analysis_results['text_to_image'][text_idx] = {
            'text': texts[text_idx],
            'correct_image': correct_img,
            'correct_image_id': correct_image_id,
            'top_k_predictions': [(img_idx, dataset.image[img_idx] if hasattr(dataset, 'image') else (dataset.image_ids[img_idx] if hasattr(dataset, 'image_ids') else img_idx), text_scores[img_idx], img_idx == correct_img) for img_idx in top_k_indices],
            'found_correct': found_correct
        }
    ...
    # 画像→テキストのエラーパターン分析
    for img_idx in range(min(100, len(scores_i2t))):
        img_scores = scores_i2t[img_idx]
        top_k_indices = np.argsort(img_scores)[::-1][:k]
        correct_texts = img2txt[img_idx]
        
        # 正解がTop-Kに入っていない場合の分析
        if not any(idx in correct_texts for idx in top_k_indices):
            correct_text_content = [texts[i] for i in correct_texts]
            predicted_text_content = [texts[i] for i in top_k_indices[:k]]  # Top-kの予測
            
            # エラーパターンを分類
            pattern = classify_error_pattern(correct_text_content, predicted_text_content)
    ...
    analysis_results['error_analysis']['error_patterns'] = error_patterns
    
    return analysis_results

def classify_error_pattern(correct_texts, predicted_texts):
    """
    エラーパターンを分類する関数
    """
    # 簡単なパターン分類（実際の用途に応じて拡張可能）
    correct_words = set()
    for text in correct_texts:
        correct_words.update(text.lower().split())
    
    predicted_words = set()
    for text in predicted_texts:
        predicted_words.update(text.lower().split())
    
    overlap = len(correct_words & predicted_words)
    total_correct = len(correct_words)
    
    if overlap == 0:
        return "完全に異なる内容"
    elif overlap / total_correct > 0.5:
        return "部分的に類似した内容"
    else:
        return "わずかに類似した内容"
...
def main(args, config, client):
    ...
    if not args.evaluate:
        ...
        if not args.debug:
            ...
            score_test_i2t_teacher, score_test_t2i_teacher, teacher_text_embeds, teacher_image_embeds, teacher_sims_matrix, teacher_min_val, teacher_max_val ,ax_epoch, teacher_top_k_analysis = evaluation(teacher_model,test_loader,device,config,ax_epoch=ax_epoch,make_sim_matrix=make_sim_matrix,teacher_model_env=teacher_model_env, analyze_top_k=args.analyze_top_k)
            print("do itm_eval")
        ...
        for epoch in range(0, config['max_epoch']):
            ...
            #枝刈り中のモデル性能を評価
            score_search_i2t, score_search_t2i, text_embeds_log, image_embeds_log, sims_matrix_log ,min_val, max_val,ax_epoch, search_top_k_analysis = evaluation(search_model, test_loader, device, config,search=True, ax_epoch=ax_epoch, make_sim_matrix=make_sim_matrix,min_val=teacher_min_val, max_val=teacher_max_val, analyze_top_k=args.analyze_top_k)
            print(f"min_val {min_val}")
            print(f"max_val {max_val}")
            print("do itm_eval")
    ...
    for epoch in range(0, config['max_epoch']):    
        ...
        score_val_i2t, score_val_t2i, text_embeds_val, image_embeds_val, sims_matrix_val, min_val, max_val,ax_epoch, val_top_k_analysis = evaluation(model_without_ddp, val_loader, device, config, min_val=teacher_min_val, max_val=teacher_max_val, analyze_top_k=args.analyze_top_k)
        score_test_i2t, score_test_t2i, text_embeds_test, image_embeds_test, sims_matrix_test, min_val, max_val,ax_epoch, test_top_k_analysis = evaluation(model_without_ddp, test_loader, device, config, make_sim_matrix=make_sim_matrix, min_val=teacher_min_val, max_val=teacher_max_val, analyze_top_k=args.analyze_top_k)
        print(f"min_val {min_val}")
        print(f"max_val {max_val}")
        #特徴表現を評価
...
if __name__ == '__main__':
    ...
    parser.add_argument('--analyze_top_k', action='store_true', help='Top-K予測の詳細分析を実行')
